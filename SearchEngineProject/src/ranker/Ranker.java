...
public class Ranker {
    ...
    public List<String> urlsFromIndexer = new ArrayList<String>();

    public Ranker(Data data, List <String> urlsFromIndxer, String searchString, String Location, Boolean wantLocationScore, Boolean wantDateScore) {
        ...
        this.urlsFromIndexer=urlsFromIndxer;
    }
    ...
    {
        double n = 0;
        for (int i = wordindex; i < occurencesOfWordsCount.size(); i= i+searchArray.length) {
            if (occurencesOfWordsCount.get(i) !=0) {
                n ++;
            }
        }
    ...
    public double idf_phrase(List<Integer> countPhraseSearch)
    {
        ...
        {
            if(countPhraseSearch.get(i) > 0)
            {n+=1 ;}
        }
        return Math.log(numofdocs/n);
    }
    public double tf_idf(int docindex, List<Integer> occurencesOfWordsCount, List<String> documentsBody) throws IOException
    // calculated for each document for the entirety of all the search string
    {
        double output = 0;
        for (int i = 0; i < searchArray.length; i++) {
            output += tf(i, occurencesOfWordsCount, docindex, documentsBody) ;
            //*idf(i, occurencesOfWordsCount);
        }
        return output;
    }
